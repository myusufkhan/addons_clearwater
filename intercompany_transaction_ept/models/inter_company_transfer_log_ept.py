...
class InterCompanyTransferLogBookEpt(models.Model):
    ...
    _description = "Inter Location Transfer Log Book"

    name = fields.Char(string="Name", copy=False)
    ict_log_date = fields.Datetime(string="Log Date", copy=False)
    ict_process = fields.Selection([("inter.company.transfer.ept", "Inter Location Transfer")],
                                   string="Application")
    operation = fields.Selection([("import", "Import"), ("reverse", "Reverse ILT"), ("invoice", "Invoice")])
...
class InterCompanyTransferLogLineEpt(models.Model):
    ...
    _description = 'Inter Location Transfer Log Line Process'

    ict_message = fields.Text(string="Message")
    ict_log_type = fields.Selection([('error', 'Error'),
                                     ('mismatch', 'Mismatch'),
                                     ('info', 'Info')], string="Log Type")
    ict_log_id = fields.Many2one('inter.company.transfer.log.book.ept', string="ILT Process Log",
                                 ondelete='cascade')
