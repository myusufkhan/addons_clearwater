...
class InterCompanyTransferEpt(models.Model):
    ...
    _description = "Inter Location Transfer EPT"
    _order = 'id desc'
    _inherit = ['barcodes.barcode_events_mixin']

    name = fields.Char(help="Name of Inter Location transfer.")
    ...
    inter_company_transfer_line_ids = fields.One2many('inter.company.transfer.line.ept',
                                                      ...
                                                      help="ILT Lines", copy=True)

    state = fields.Selection([('draft', 'Draft'),
                              ('processed', 'Processed'),
                              ('cancel', 'Cancelled')],
                             string='State', copy=False, default='draft', help="State of ILT.")
    type = fields.Selection([('ict', 'ILT'), ('ict_reverse', 'Reverse ILT'),
    ...
    log_ids = fields.One2many('inter.company.transfer.log.book.ept', 'inter_company_transfer_id',
                              string="Inter Location Log", help="Logs of ILT.")
    log_count = fields.Integer(compute='_compute_log_ids', help="Count of Logs.")
    message = fields.Char("Message", copy=False, help="Message for ILT.")
    processed_date = fields.Datetime("Processed Date", copy=False,
                                     help="Date when ILT is processed.")
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if view_type in ['form', 'tree']:
            if context.get('default_type', 'ict_reverse') == 'ict_reverse':
                for node in doc.xpath("//tree[@string='Inter Location Transfer']"):
                    node.set('create', 'false')
                for node in doc.xpath("//form[@string='Inter Location Transfer']"):
                    node.set('create', 'false')
    ...
    def action_cancel(self):
        ...
        if self.state == 'processed':
            if pickings.filtered(lambda x: x.state == 'done'):
                raise Warning("You can not cancel Inter Location Transfer which has done "
                              "pickings.")
            if invoices.filtered(lambda x: x.state == 'post'):
                raise Warning("You can not cancel Inter Location Transfer which has posted "
                              "invoices.")
            self.sale_order_ids.action_cancel()
            self.purchase_order_ids.button_cancel()
            pickings.action_cancel()
            invoices.button_cancel()
        self.write({'state':'cancel', 'message' : 'ILT has been cancelled by %s' % (
            self.env.user.name)})
        ...
        for record in self:
            if record.with_context(context).check_user_validation():
                if not record.inter_company_transfer_line_ids:
                    msg = "Please Add the Product to Process ILT."
                ...
                if record.type == "internal":
                    ...
                    if internal_transfer:
                        record.write({'state':'processed', 'processed_date':datetime.today(),
                                      'message':'ILT processed successfully by %s' % (
                                          self.env.user.name)})
                    return True
                ...
                record.write({'state':'processed', 'processed_date':datetime.today(),
                              'message':'ILT processed successfully by %s' % (self.env.user.name)})
        ...
        for record in self:
            if record.source_warehouse_id.company_id not in self.env.user.company_ids:
                if record.source_warehouse_id.company_id not in\
                    ...
                    raise ValidationError("""User '%s' can not process this Inter Location Transfer.\n User
